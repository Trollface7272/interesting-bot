...
module.exports = {
    async getCtbpp(message, bot) {
        ...
        var rating, performance, title, accuracy, s300, s100, s50, miss, s50miss, difficulty, userCombo, approachRate, retries, footer, playDate, now
        await osuApi.getUserRecent({u: name, m: '2', a: '1'}).then(async score => {
            ...
            await osuApi.getBeatmaps({b: beatmapId, m: '2', a: '1'}).then(async beatmap => {
                ...
                .setDescription(`
                    ▸ ${rankingEmoji} ▸ **${(Math.round(performance * 100) / 100).toFixed(2)}PP** ▸ ${accuracy}%
